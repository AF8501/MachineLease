package com.example.gyx.jixiezulin.ModelList.data;
 ...
 * 数据格式：支持两种数据格式，分别是Json和List<String>。注意：Json 的name字段固定使用用"models"。
 * 使用方法：new BrandData()，然后setJson或者setList,然后getItemList()。
 * 示例代码：
 * BrandData data = new BrandData();
 * String json =  "{'models':['HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH','HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH','HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH']}";
 * data.setJson(json);
 */
public class ModelData
{
	...
	public ModelData(String json) {
		setJson(json);
	}

	public ModelData(List<String> list) {
		setList(list);
	}
	...
	public final static String modelJson = "{'models':['HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH','HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH','HD10VV','HD12VT','PC56-U','PC56-T','PW78-P','UV11HH']}";
	private List<ContactItemInterface> getSampleContactList(String josnData)
	{
		...
		try
		{
			...
			JSONArray ja1 = jo1.getJSONArray("models");
			...
			for(int i = 0; i < ja1.length(); i++)
			{
				...
				list.add(new ModelItem(equipmentName, Pinyin4jUtil.converterToSpell(equipmentName)));
			}
		} 
